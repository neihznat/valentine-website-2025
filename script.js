...
function handleFirstQuestionClick(button) {
    ...
    setKissCount(next);
    // Spawn one floating emoji (3x speed) for Yes vs No
    const emoji = button.id === 'yesBtn1'
        ? (config.firstQuestionYesEmoji || '❤️')
        : (config.firstQuestionNoEmoji || '😢');
    createFloatingFirstQuestionEmoji(emoji);
    moveButton(button);
}

function createFloatingFirstQuestionEmoji(emoji) {
    const container = document.querySelector('.floating-elements');
    const div = document.createElement('div');
    div.className = 'floating-first-q-emoji';
    div.textContent = emoji;
    setRandomPositionAndDelayOnly(div);
