...
function createOneFloatingFirstQuestionEmoji(emoji) {
...
function burstFirstQuestionEmojis(emoji) {
    ...
    const tick = () => {
        ...
        for (let i = 0; i < USER_FLOATER_BATCH_SIZE; i++) {
            const div = document.createElement('div');
            div.className = 'floating-first-q-emoji';
            div.textContent = emoji;
            setRandomPositionNoDelay(div);
        ...
        setTimeout(tick, USER_FLOATER_INTERVAL_MS);
    };
    tick();
}
...
const USER_FLOATER_BURST_MS = 3000;       // Spawn for 3 seconds
const USER_FLOATER_INTERVAL_MS = 50;       // Every 50ms
const USER_FLOATER_BATCH_SIZE = 25;       // Massive amount per batch
